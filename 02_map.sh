...
#     bowtie2 -p 2 --no-unal -q -x $genome_location/ce11 -U $f -S ./$SRR_exp/bam/${target_name}.sam > ./$SRR_exp/bam/${target_name}_alignment_report_bw2.txt
     bwa aln -t 8 $genome_location/ce11bwaidx $f > $working_path/$SRR_exp/bam/${target_name}.sai
     ...
     samtools view -S -b -q 20 $working_path/$SRR_exp/bam/${target_name}.sam > $working_path/$SRR_exp/bam/${target_name}.bam
