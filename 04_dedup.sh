...
FILES=$(find $working_path/aln/ -type f -name "${SRR_exp}_*_sort.bam")
...
for f in ${FILES[@]}
  ...
  if [ ! -f "$working_path/filt/${target_name}_dedup_filt_sort.bam" ]; then
    ...
    picard-tools MarkDuplicates I=$f O=$working_path/dedup/${target_name}_dedup.bam M=$working_path/dedup/${target_name}_dedup.txt REMOVE_DUPLICATES=true REMOVE_SEQUENCING_DUPLICATES=true TMP_DIR=$SCRATCH VALIDATION_STRINGENCY=LENIENT
