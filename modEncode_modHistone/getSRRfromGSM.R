...
geoTable<-read.csv(file=paste0(workDir,"/modEncode_chromatinChipSeq_modHistone.csv"),
...
mergeGeoSrrTables1<-function(geoTable,srrTable){
  ...
  for(i in 1:nrow(geoTable)){
    ...
    dfline<-cbind(geoTable[i,], data.frame(input_1=NA, ip_1=NA, name_1=NA,
                                           ...
                                           group_1=i))
    dflines<-as.data.frame(do.call("rbind",replicate(length(geoNums)/2,dfline,simplify=F)))

    idx<-match(geoNums,srrTable$GEO)
    if(length(geoNums)==1){
      print(paste0("no geo IDs: ",i))
      failed<-c(failed,i)
      next;
...
write.table(fullTable1, file=paste0(workDir,"/modEncode_chromatinChipSeq_modHistone_fullTable.tsv"), sep="\t", row.names=F)
