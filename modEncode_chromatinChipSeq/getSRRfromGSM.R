...
geoToSra<-function(geoNum){
  df<-tryCatch(
  {
   ...
   name<-XML::xmlValue(XML::getNodeSet(ex,"//SAMPLE /TITLE"))
   strain<-XML::xmlValue(XML::getNodeSet(ex,"//SAMPLE/SAMPLE_ATTRIBUTES/SAMPLE_ATTRIBUTE[TAG='strain']/VALUE"))
   stage<-XML::xmlValue(XML::getNodeSet(ex,"//SAMPLE/SAMPLE_ATTRIBUTES/SAMPLE_ATTRIBUTE[TAG='Stage']/VALUE"))
...
processGeoNumList<-function(geoNums){
  ...
  df<-do.call(rbind,df)
  #df$input<-grepl("input DNA",df$name)
  df$antibody<-sapply(strsplit(gsub("^seq-","",df$name),"_"),"[",1)
  df$input<-grepl("input DNA",df$name) | df$antibody=="NA"
  df$ip<-df$antibody!="NA"
  df$rep1<-grepl("_1$",df$name,ignore.case=T) | grepl("_1_",df$name,ignore.case=T)
  df$rep2<-grepl("_2$",df$name,ignore.case=T) | grepl("_2_",df$name,ignore.case=T)
  #manual changes:
  df$rep1[df$name=="seq-JL00001_DPY27_N2_L3_3"]<-T
  df$rep1[df$name=="seq-JA00011_LIN35_N2_L3_4"]<-T
  df$rep2[df$name=="seq-AB9050_H3K36me3_N2_L3_7"]<-T
  df$rep1[df$input==T]<-F
  df$rep2[df$input==T]<-F
  return(df)
}
...
geoTable<-read.csv(file=paste0(workDir,"/modEncode_chromatinChipSeq_mix.csv"),
...
srrTable[(srrTable$rep1 + srrTable$rep2)!=1,] # all the inputs
...
mergeGeoSrrTables1<-function(geoTable,srrTable){
  ...
  for(i in 1:nrow(geoTable)){
    ...
    dfline<-cbind(geoTable[i,], data.frame(input_1=NA, ip_1=NA, name_1=NA,
                                           ...
                                           group_1=i))
    #inputs in this dataset are not matched, so need to merge inputs and have one line per replicate ip
    numIPs<-sum(srrTable$ip[match(geoNums,srrTable$GEO)])
    dflines<-as.data.frame(do.call("rbind",replicate(numIPs,
                                                     dfline,simplify=F)))
    ...
    if(length(geoNums)==1){
      ...
      tryCatch({ dflines$input_1<-paste0(srrTable$SRR[idx][srrTable$input[idx]],collapse=" ")},
              error=function(e){
      ...
      dflines$name_1<-rbind(paste0(srrTable$name[idx][srrTable$rep1[idx]],
                            ...
                            ifelse(any(srrTable$rep2[idx]==TRUE),
                               paste0(srrTable$name[idx][srrTable$rep2[idx]],
                                     collapse=";"),
                               NA))
    } else if(length(geoNums)==2 & numIPs>0) {
      print(paste("processing 1 replicate",i,paste0(geoNums,collapse=";")))
      dflines$input_1<-paste0(srrTable$SRR[idx][srrTable$input[idx]],collapse=" ")
      dflines$ip_1<-srrTable$SRR[idx][srrTable$ip[idx]]
      dflines$name_1<-paste0(srrTable$name[idx][srrTable$rep1[idx]],
                               collapse=";")
    } else {
      print(paste("failed for unknown reason:",paste(geoNums,collapse=" "),"line",i))
      failed<-c(failed,i)
    }
    if(numIPs>0){
      dflines$replicate_1<-paste0("rep",1:(numIPs))
...
write.table(fullTable1, file=paste0(workDir,"/modEncode_chromatinChipSeq_mix_fullTable.tsv"), sep="\t", row.names=F)
